...
### Test Public Lobby:
```bash
curl -X GET https://codecheck-24.preview.emergentagent.com/api/lobby/items
```

### Test Create Folder (Super Admin):
```bash
curl -X POST https://codecheck-24.preview.emergentagent.com/api/folders \
...
```bash
curl -X POST https://codecheck-24.preview.emergentagent.com/api/folders/YEAR_FOLDER_UUID/upload-excel \
  -H "Authorization: Bearer YOUR_SUPER_ADMIN_TOKEN" \
...
```bash
curl -X POST https://codecheck-24.preview.emergentagent.com/api/items \
